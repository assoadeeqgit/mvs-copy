...
if (!empty($category)) {
    try {
        $sql = "SELECT p.product_id, p.name, p.description, p.price, p.discounted_price, p.stock_quantity, p.is_published, 
                ...
                WHERE c.name = :category AND p.is_published = 1
        ...
        if (empty($products)) {
            ...
            $sql = "SELECT COUNT(*) FROM categories WHERE name = :category";
            $stmt = $dbh->prepare($sql);
            $stmt->execute(['category' => $category]);
        ...
        $_SESSION['error_message'] = 'Error loading category products.';
    }
}
...
<head>
    ...
    <title>Products in <?php echo htmlspecialchars($category ?: 'Category'); ?> - MultiVendor Marketplace</title>
...
<body class="bg-gray-50 font-sans">
    ...
    <section class="container mx-auto px-4 py-12">
        ...
        <h2 class="text-3xl font-bold mb-8">
            Products in "<?php echo htmlspecialchars($category ?: 'Category'); ?>"
        </h2>

        <?php if (empty($category)): ?>
        <div class="text-center text-gray-600">
            <p class="text-lg">Please select a category to view products.</p>
            <p class="mt-2">Browse our <a href="index.php#categories"
                    class="text-primary hover:underline">categories</a>.</p>
        </div>
        <?php elseif ($invalidCategory): ?>
        <div class="text-center text-gray-600">
            <p class="text-lg">Invalid category "<?php echo htmlspecialchars($category); ?>"</p>
            <p class="mt-2">Please select a valid category from the <a href="index.php#categories"
                    class="text-primary hover:underline">categories</a> menu.</p>
        </div>
        <?php elseif ($noResults): ?>
        <div class="text-center text-gray-600">
            <p class="text-lg">No products found in "<?php echo htmlspecialchars($category); ?>"</p>
            <p class="mt-2">Try browsing other <a href="index.php#categories"
                    class="text-primary hover:underline">categories</a>.</p>
        </div>
        <?php else: ?>
